        ...
        <li>
            <form action="/backoffice/referenda">
                ...
                <select name="keyword" onchange="this.form.submit()">
                    ...
                    <option <?php if($_GET){ if ($_GET['keyword'] == 'published') { ?>selected="true"
                            <?php }}; ?> value="published">published
                    </option>
                    <option <?php if($_GET){ if ($_GET['keyword'] == 'unpublished') { ?>selected="true"
                            <?php }}; ?> value="unpublished">unpublished
                    </option>
                </select>
            </form>
        </li>
    ...
    <table class="table">
        ...
        @foreach($referenda as $item)
            <tr>
                ...
                <td><a href="/backoffice/referenda/{{$item->id}}">{{$item->published? "Published": "Unpublished"}}</a>
                </td>
                <td><a href="/backoffice/referenda/{{$item->id}}">{{$item->isClosed? "Closed": "Open"}}</a></td>
                <td><a href="/backoffice/referenda/{{$item->id}}/edit"><i class="fa fa-pencil"
                                                                          aria-hidden="true"></i></a></td>
                <td><a href="/backoffice/referenda/{{$item->id}}"><i class="fa fa-eye" aria-hidden="true"></i></a></td>
                <td>
                    <form id="delete_form{{$item->id}}" action="{{ URL::route('referenda.destroy',$item->id) }}"
                          method="POST">
